...
MFRC522DriverPinSimple ss_pin(5);     // SS/CS -> GPIO 5
...
bool writeBlock (byte blockAddr, byte *data16) {
  setKeyDefault();    // Authenticate with Key A (0x60), Authenticate and unlock the
  if (mfrc522.PCD_Authenticate (0x60, blockAddr, &key, & (mfrc522.uid)) != 0) {
    Serial.print(F("Auth failed (write) for block")); Serial.println(blockAddr);
    return false;
}
if (mfrc522.MIFARE_Write (blockAddr, data16, 16) != 0) {
...
void writeLongEmail(byte block1, byte block2, String data) {
  ...
  status = mfrc522.MIFARE_Write(block2, buffer, 16);
  if (status != MFRC522::STATUS_OK) {
...
String readLongName(byte block1, byte block2) {
  ...
  mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A,
                            block1, &key, &(mfrc522.uid));
  mfrc522.MIFARE_Read(block1, buffer, &size);
  ...
  mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A,
                            block2, &key, &(mfrc522.uid));
  mfrc522.MIFARE_Read(block2, buffer, &size);
...
mfrc522.PICC_HaltA();      // tell the tag we're done (halt)
mfrc522.PCD_StopCrypto1(); // stop encryption session on the reader
delay(1000);
